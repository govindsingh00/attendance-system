...
def admin_home(request):
    if request.session.has_key("email"):
        ...
        if ut == "admin":
            ...
            return render(request,"admin-dashboard.html",{"data":adm,"data1":pic})
        else:
            return HttpResponseRedirect("/autherror/")
    else:
        return HttpResponseRedirect("/autherror/")
...
def teacher_home(request):
    if request.session.has_key("email"):
        ...
        if ut == "teacher":
            ...
            return render(request, "teacher-dashboard.html", {"data": tchr, "data1": pic})
        else:
            return HttpResponseRedirect("/autherror/")
    else:
        return HttpResponseRedirect("/autherror/")
...
def student_home(request):
    if request.session.has_key("email"):
        ...
        if ut == "student":
            ...
            pic = Photodata.objects.filter(email=em)
            return render(request,"student-dashboard.html",{"data":std,"data1":pic})
        else:
            return HttpResponseRedirect("/autherror/")
    else:
        return HttpResponseRedirect("/autherror/")
...
def student_attendance(request):
    if not request.session.has_key("email") or request.session.get("ut") != "student":
        return HttpResponseRedirect("/autherror/")
    em = request.session["email"]
    student = Studentdata.objects.filter(email=em).first()
    records = AttendanceRecord.objects.filter(student=student).select_related("course") if student else []
    by_course = {}
    for r in records:
        cid = r.course_id
